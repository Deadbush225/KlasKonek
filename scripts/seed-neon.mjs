...
function chooseOccupation(years) {
	if (years >= 16)
		return pickOne(["Head Teacher", "School Principal", "Master Teacher II"]);
	if (years >= 11)
		return pickOne(["Master Teacher I", "Master Teacher II", "Teacher III"]);
...
function chooseQualification(years) {
	if (years >= 18)
		return pickOne(["Doctoral Units", "Doctoral Degree", "Master's Degree"]);
	if (years >= 10)
		return pickOne([
			"Master's Degree",
...
function chooseParticipationStatus(years) {
	...
	if (years >= 3)
		return pickOne(["Applied", "Active Participant", "Interested"]);
	return pickOne(["Interested", "Applied", "Not Yet Participated"]);
}

function chooseRegion() {
	const entries = Object.entries(REGION_WEIGHT_MODEL).map(
		([region, weight]) => ({
			value: region,
			weight,
		}),
...
function chooseSchoolType(region) {
	const profile =
		SCHOOL_TYPE_DISTRIBUTION_BY_REGION[region] ??
		SCHOOL_TYPE_DISTRIBUTION_BY_REGION.default;
...
function chooseSubjectsBySchoolType(schoolType) {
	...
	const additionalCandidates = SUBJECT_POOL.filter(
...
function buildSchoolName({ division, schoolType, ordinal }) {
	const template = pickOne(
		SCHOOL_NAME_TEMPLATES[schoolType] ?? SCHOOL_NAME_TEMPLATES.urban,
	);
...
async function seedMockTeacherProfiles(totalCount = 1000) {
	const regionCount = new Map(
		Object.keys(REGION_DIVISION_POOL).map((region) => [region, 0]),
	);
	...
	console.log(
		"Region distribution sample:",
		Object.fromEntries(regionCount.entries()),
	);
	console.log(
		"School type distribution sample:",
		Object.fromEntries(schoolTypeCount.entries()),
	);
}
...
async function upsertProfile(profile) {
	...
	const starParticipationStatus =
		profile.starParticipationStatus ?? "Active Participant";
...
async function insertResourceIfMissing(resource) {
	const existing = await pool.query(
		"select id from resources where title = $1 limit 1",
		[resource.title],
	);
...
const adrielId = await upsertProfile({
	...
	trainingHistory: [
		"2025 AI in STEM Fellowship",
		"2024 Regional Action Research Bootcamp",
	],
...
const christineId = await upsertProfile({
	...
	trainingHistory: [
		"2023 Regional Science Leadership Summit",
		"2024 KlasKonek Mentor Training",
	],
	starParticipationStatus: "Alumni",
	dataQualityScore: 96,
	years: 9,
...
const MOCK_PROFILE_COUNT = Number.parseInt(
	process.env.MOCK_PROFILE_COUNT ?? "1000",
	10,
);
...
await insertResourceIfMissing({
	...
	description:
		"Practical checklist for introducing safe and responsible AI usage in class.",
...
await insertResourceIfMissing({
	...
	description:
		"Activity sheets and facilitation notes for collaborative problem solving.",
...
await insertResourceIfMissing({
	...
	description:
		"Operations playbook for scaling extension activities across schools.",
...
const MOCK_PROGRAMS = [
	{
		...
		status: "completed",
		notes:
			"Annual convention featuring keynote speakers, parallel sessions on STEM pedagogy, and poster presentations from regional awardees.",
	},
	{
		...
		status: "completed",
		notes:
			"Intensive 5-day in-person workshop covering action research design, data collection, and manuscript preparation.",
	},
	{
		...
		status: "completed",
		notes:
			"Three-day regional summit in Laguna. Focused on inquiry-based science teaching and localized curriculum development.",
	},
	{
		...
		status: "ongoing",
		notes:
			"6-month virtual mentoring program for BARMM science teachers. Monthly check-ins and peer review circles.",
	},
	{
		...
		status: "ongoing",
		notes:
			"Pairing urban schools in Baguio with rural schools in Ilocos Norte for shared lab resources and co-teaching sessions.",
	},
	{
		...
		status: "scheduled",
		notes:
			"Bridging program for teachers transitioning from General Science to specialized Physics or Chemistry tracks.",
	},
	{
		...
		status: "scheduled",
		notes:
			"Learning Action Cell sessions on formative assessment strategies in mathematics. Facilitated by regional master teachers.",
	},
	{
		...
		status: "scheduled",
		notes:
			"Two-week asynchronous online course covering spreadsheet analysis, basic statistics, and data visualization for classroom use.",
	},
	{
		...
		status: "completed",
		notes:
			"Orientation program for newly hired science and math teachers. Covers program objectives, classroom management, and professional development pathways.",
	},
	{
		...
		status: "scheduled",
		notes:
			"5-day workshop covering laboratory-based inquiry, research ethics, and community engagement for STEM outreach.",
	},
	{
		...
		status: "completed",
		notes:
			"Two-day summit featuring best practices in STEM education for Bicol Region teachers. Included hands-on workshops and research poster sessions.",
	},
	{
		...
		status: "ongoing",
		notes:
			"4-month cohort-based mentoring with bi-weekly virtual sessions and a culminating research symposium.",
	},
];
